	...
	switch( $action ) {
		...
		case 'view':
			...
			if ( ! $user ) {
				...
				$query = "SELECT DATE_FORMAT( date_time, '%Y-%m-%d' ) as formatted_date FROM " . $wpdb->base_prefix . "reports_post_activity WHERE user_ID = '" . $user_ID . "' AND post_type = 'page' AND date_time > '" . reports_days_ago($total_days_safe,'Y-m-d') . " 00:00:00'";
