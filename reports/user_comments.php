<?php
/*
Plugin Name: Report - User Comments
Plugin URI: 
Description:
...
reports_add_report('User Comments','user-comments','Displays comment activity for a user');

if ( $_GET['report'] == 'user-comments' ) {
...
function report_user_comments_ouput(){
	...
	switch( $_GET[ 'report-action' ] ) {
		...
		default:
			?>
			<form name="report" method="post" action="ms-admin.php?page=reports&action=view-report&report=user-comments&report-action=view">
		...
		case "view":
			...
			if ( $user_count == '0' ) {
				...
				$query = "SELECT DATE_FORMAT( date_time, '%Y-%m-%d' ) as formatted_date FROM " . $wpdb->base_prefix . "reports_comment_activity WHERE user_ID = '" . $user_ID . "' AND date_time > '" . reports_days_ago($total_days_safe,'Y-m-d') . " 00:00:00'";
				...
				$g->area_hollow(1,3,4,'#3357A0', __('comment(s)'),10);
